    ...
    for (int i = 0; modifiedText[i] != '\0'; i++)
        {//text parameter
            ...
            if ((rawText[i]=='<' && rawText[i+1]=='i'))
            {
                ...
                modifiedText.replace(i-5,lenght+2,"\0");
...
void Dialogue::draw(Game &game)
{
    ...
    printText(font,TextSubtext(TextFormat("%s", game.storedDialogue[mState].text.c_str()), 0, game.textFrame/5),{mRec.x +20, mRec.y + textMeasure.y / 2},mFontSize,1,mTextColor,game);
